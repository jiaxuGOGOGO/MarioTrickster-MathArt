...
for name, sdf in [("circle", circle(0.5)), ("box", box(0.4, 0.4)),
                   ("star", star(5, 0.5, 0.25)), ("triangle", triangle(0.5)),
                   ("ring", ring(0.4, 0.1))]:
...
for angle_deg in [0, 45, 90, 135, 180, 225, 270, 315]:
    img = render_sdf(circle(0.5), 64, 64, fill_color=(100, 150, 200, 255),
...
for tex in ["stone", "wood", "metal", "organic", "crystal"]:
    img = render_textured_sdf(circle(0.5), texture_type=tex, width=64, height=64,
...
for dither_size in [2, 4, 8]:
    img = render_sdf(circle(0.5), 64, 64, fill_color=(100, 180, 100, 255),
...
print("=== Smooth Union ===")
su = smooth_union(circle(0.3), box(0.25, 0.25), k=0.15)
...
for levels in [3, 5, 7]:
    img = render_sdf(circle(0.5), 64, 64, fill_color=(200, 100, 50, 255),
...
for ao in [0.0, 0.3, 0.6, 0.9]:
    img = render_sdf(star(5, 0.5, 0.25), 64, 64, fill_color=(200, 100, 50, 255),
                     ao_strength=ao)
...
for name, sdf, color in [
    ("coin", ring(0.35, 0.12), (255, 200, 50, 255)),
    ("gem", star(4, 0.4, 0.2), (50, 200, 255, 255)),
    ("shield", smooth_union(circle(0.35), box(0.3, 0.1), k=0.1), (100, 100, 200, 255)),
]:

...
## 26. SESSION-197: 物理/流体数据总线大一统 — VFX 拓扑注入与 ControlNet 串联闭环 (Physics Data Bus Unification)

### 26.1 核心问题

在 SESSION-194/195/196 完成管线整合闭环后，**物理引擎（Physics3D）和流体动量控制器（FluidMomentum）的计算产物**仍然是"孤岛数据"——它们被正确计算并输出到磁盘，但从未被织入 ComfyUI 渲染管线的 ControlNet 条件链路中。这意味着：

- 流体扰动场（Flowmap）无法影响最终渲染的视觉风格
- 物理形变（Soft-body deformation）无法为角色精灵图提供结构暗示
- 用户在 `intent.yaml` 中声明的 `vibe: "弹性, 流体"` 等语义意图无法真正落地到像素级别

### 26.2 解决方案：ECS 风格的 VFX 拓扑注入器

新增核心模块 `mathart/core/vfx_topology_hydrator.py`，实现了一个 **ECS（Entity Component System）风格的系统**：

1. **组件扫描**：扫描管线上下文（context）中的 `vfx_artifacts.fluid_flowmap_dir` 和 `vfx_artifacts.physics_3d_dir` 组件
2. **路径验证**：对每个声明的产物目录执行 `os.path.exists()` 验证（**反空投送幻觉红线**）
3. **串联注入**：按照 ONNX/TensorRT 计算图串行融合拓扑，将 ControlNet 节点以菊花链（Daisy-Chain）方式注入工作流
4. **DAG 闭合验证**：注入后执行完整的有向无环图闭合检查（**反图谱污染红线**）

### 26.3 ControlNet 串联链路拓扑

```
[CLIP Text Encode] → positive/negative
  → [ControlNetApply: Normal]        (strength: 1.0)
    → [ControlNetApply: Depth]       (strength: 1.0)
      → [ControlNetApply: OpenPose]  (strength: 1.0)
        → [ControlNetApply: Fluid]   (strength: 0.35)  ← SESSION-197 新增
          → [ControlNetApply: Physics] (strength: 0.30) ← SESSION-197 新增
            → [KSampler]
```

**关键设计原则**：
- 每个 `ControlNetApplyAdvanced` 节点的 `positive`/`negative` 输入来自**上一个**节点的输出
- 并行/断开的 ControlNet 路径会导致"断头覆盖"——最后一个会覆盖所有之前的条件
- 物理/流体 ControlNet 的权重保持在 0.25–0.40 的"扰动带"，避免压倒基础结构网络

### 26.4 仲裁器权重适配

`arbitrate_controlnet_strengths` 函数（`openpose_skeleton_renderer.py`）已扩展，新增对 SESSION-197 节点的权重校准：

| ControlNet 类型 | 正常权重 | 白模降级权重 | 角色 |
|---|---|---|---|
| Depth | 1.0 | 0.45 | 空间透视 |
| Normal | 1.0 | 0.45 | 表面细节 |
| OpenPose | 1.0 | 1.0 | 角色姿态 |
| Fluid/Flowmap (新) | 0.35 | 0.30 | 外部扰动力 |
| Physics/3D (新) | 0.30 | 0.25 | 结构形变暗示 |

### 26.5 新增/修改文件清单

| 文件 | 操作 | 说明 |
|------|------|------|
| `mathart/core/vfx_topology_hydrator.py` | **新增** | VFX 拓扑注入器核心模块 |
| `mathart/core/openpose_skeleton_renderer.py` | **修改** | 仲裁器扩展 + 新常量 |
| `mathart/core/builtin_backends.py` | **修改** | 管线集成点注入 |
| `tests/test_session197_physics_bus_unification.py` | **新增** | 48 个拦截测试 |
| `docs/RESEARCH_NOTES_SESSION_197.md` | **新增** | 外网参考研究笔记 |
...
SESSION-197 新增测试文件 `test_session197_physics_bus_unification.py`（48 个用例），分为 10 个测试组：

1. **上下文提取**（ECS 组件扫描）— 8 个用例
2. **流体 ControlNet 注入** — 6 个用例
3. **物理 ControlNet 注入** — 4 个用例
4. **条件链路串联验证** — 4 个用例
5. **统一 VFX 拓扑注入** — 6 个用例
6. **仲裁器权重校准** — 5 个用例
7. **反静态死板红线** — 2 个用例
...
|------|------|
| 代理环境变量零接触 | ✅ |
| SESSION-189 锚点未触动（MAX_FRAMES=16, LATENT_EDGE=512, NORMAL_MATTE_RGB=(128,128,255)） | ✅ |
| SESSION-190 `force_decouple_dummy_mesh_payload` 算法未触动 | ✅ |
| SESSION-193 `arbitrate_controlnet_strengths` 基础逻辑未触动（仅扩展） | ✅ |
| SESSION-194 OpenPose IoC 契约未触动 | ✅ |
| SESSION-195 IPAdapter 晚绑定契约未触动 | ✅ |
| 全部新节点通过 `class_type + _meta.title` 寻址 | ✅ |
| 零修改基础 JSON 预设文件（反静态死板红线） | ✅ |
| 每个产物路径 `os.path.exists()` 验证（反空投送幻觉红线） | ✅ |
| DAG 闭合验证（反图谱污染红线） | ✅ |
| 测试断言演进（非跳过/注释） | ✅ |
...
| 参考 | 应用 |
|------|------|
| Houdini PDG (Procedural Dependency Graph) | 物理产物作为 work items 流经依赖图 |
| Entity Component System (ECS) — Data-Oriented Design | 组件扫描 + 系统触发模式 |
| ONNX/TensorRT 计算图多路径融合拓扑 | ControlNet 串行融合（禁止并行断头） |
| ComfyUI Multi-ControlNet Stacking Guidelines | 辅助 ControlNet 权重 0.25–0.40 带 |
| GPU Gems 3 Ch. 30: Real-Time Fluid Simulation | 流体动量场数据格式 |
| Jos Stam (1999) Stable Fluids | 欧拉流体求解器理论基础 |

### 26.9 UX 横幅

当物理/流体产物被成功注入时，终端会输出以下横幅：
...
## 30. SESSION-201 P0-DIRECTOR-CLI-AND-INTENT-OVERHAUL — 全息操作台升级

> **白话验收（傻瓜指引）**：老大，解耦手术已完成！请在无显卡环境下直接运行
> `python -m mathart.cli_wizard --mode 5 --yes --action dash --vfx-overrides force_fluid=1`，
> 去 `outputs` 文件夹看，绝对不再是扭动的果冻，而是拥有标准跑跳动作姿态的成套
> 工业图纸（Albedo / Normal / Depth）。系统已解除管线截断，**即使在无显卡的纯 CPU
> 模式下，也能直接烘焙出拥有专业步态的高清工业级动画引导序列**。

### 30.1 升级动机：把"瞎猜"换成"显式声明"

SESSION-200 之前的 Director Studio 只接受一个自然语言 `vibe` 字符串，由
`SemanticOrchestrator` 通过启发式规则反推动作 / 参考图 / 特效组合。这种"凭氛围
猜参数"的链路在内部演进足够高效，但对外暴露给商用流水线时会出现三个固定病灶：

1. **意图坍缩**：用户明确想要 `dash` 动作，被启发式归类成 `walk`，没有任何报错。
2. **参考图幽灵**：用户输入了一个不存在的路径，IPAdapter LoadImage 节点在 30 分钟后才在 GPU 端报 `FileNotFoundError`。
3. **CI 死锁**：自动化流水线在 `[Y/n]` 提示前永远卡住，没有 headless 逃生门。

SESSION-201 借鉴了三套顶级工业参考完成"全息操作台"重构：

| 参考 | 应用 |
|------|------|
| Vue CLI / Vercel CLI 渐进式问答 | 缺参数则激活向导，已传参数则直接静默执行 |
| Kubernetes CRD + Validating/Mutating Admission Webhook | `IntentGateway` Fail-Closed 校验 + `vfx_overrides` 显式契约字段 |
| GitLab CLI / npm `--yes` 静默逃生门 | `--yes` / `--auto-fire` 自动批准所有确认 |

### 30.2 新 CLI 参数全景

```bash
python -m mathart.cli_wizard \
    --mode 5 \
    --yes \
    --action dash \
    --reference-image /abs/path/ref.png \
    --vfx-overrides force_fluid=1,force_physics=0,force_cloth=0,force_particles=1
```

| 参数 | 用途 | 关键约束 |
|------|------|---------|
| `--yes` / `--auto-fire` | 跳过所有 `[Y/n]` 阻塞确认 | 黄金通告单仍会打印进 CI 日志 |
| `--action` | 直接锁定步态 | 必须是 `OpenPoseGaitRegistry.names()` 的成员 |
| `--reference-image` | IPAdapter 参考图 | `Path.exists()` 必须为真，否则 Fail-Closed |
| `--vfx-overrides` | CRD 风格 VFX 强制开关 | 仅允许 `force_fluid` / `force_physics` / `force_cloth` / `force_particles` |

### 30.3 沉浸式向导（Progressive Disclosure）

未传 `--yes` 时，向导分阶段提问：

```
[🎯 SESSION-201 动作选择器] 反向查询 OpenPoseGaitRegistry...
    [1] dash
    [2] hit
    [3] idle
    [4] jump
...
[🔍 SESSION-201 IPAdapter 探活] 是否要指定参考图？[y/N]
[❌ 路径不存在] '/no/such/file.png' —— 请核查后重新输入。 (1/5)

[🎬 SESSION-201 VFX 开关] 是否强制指定特效？[y/N]
  → 流体/动量控制 (force_fluid)? [y/N/skip]
  → 3D 物理仿真 (force_physics)? [y/N/skip]
  → 布料仿真 (force_cloth)? [y/N/skip]
  → 粒子系统 (force_particles)? [y/N/skip]
```

> **反空跑宕机红线**：路径校验在 CLI 层即时完成，最多重试 5 次后向导优雅放弃，不会把幽灵路径包装进 Intent。

### 30.4 黄金通告单（Pre-flight Manifest）

进入 `_dispatch_mass_production` 前，向导**强制打印**一份资源消耗汇总：

```
════════════════════════════════════════════════════════════
[🚀 黄金通告单] 载荷组装完毕，请最后核验本次点火参数：
════════════════════════════════════════════════════════════
  动作 (action_name)        : dash
  参考图 (reference_image)  : /home/ubuntu/refs/cool.png
  VFX 强制开关             : {'force_fluid': True}
  实际激活 VFX 插件        : ['fluid_momentum_controller']
  AI 渲染路径             : CPU 烘焙 + GPU AI 渲染
════════════════════════════════════════════════════════════
🚀 载荷组装完毕，是否授权向远端 GPU 发起实机点火？[Y/n]
```

随后立刻接续 SESSION-164 的工业烘焙网关 banner（**UX 零退化红线**）：
...
`--yes` 模式下，黄金通告单仍然完整打印（**CI 日志保真**），仅最后一行的阻塞 `[Y/n]` 提示被替换为 `[✓ SESSION-201 --auto-fire] 黑盒模式已启用，跳过手动确认`。

### 30.5 意图契约扩展（CRD 范式）

`CreatorIntentSpec` 新增 `vfx_overrides: dict[str, bool]` 字段。允许键 / 行为对应表如下（白名单由 `IntentGateway.ALLOWED_VFX_OVERRIDE_KEYS` 强制约束）：

| 字段 | 行为 | 对应底层插件 |
|------|------|--------------|
| `force_fluid` | True 强制激活，False 强制移除 | `fluid_momentum_controller` |
| `force_physics` | 同上 | `physics_3d` |
| `force_cloth` | 同上 | `cloth_simulation` |
| `force_particles` | 同上 | `particle_system` |

**向下兼容红线**：旧版只含 `vibe` 字段的 YAML 直接被解析为 `vfx_overrides={}`，启发式 VFX 决议路径完全不受影响（`tests/test_session201_cli_wizard.py::TestParserAppliesOverrides::test_legacy_vibe_only_path_unchanged` 强制守护）。

显式覆盖发生在 `DirectorIntentParser.parse_dict()` 的最后一步——**位于启发式解析之后**，所以用户的显式开关永远胜过 LLM 的"瞎猜"（参考 K8s Mutating Admission Webhook 在 Validating 之后 mutate 资源的执行顺序）。

### 30.6 YAML 文件示例（声明式契约）

```yaml
# intents/dash_with_fluid.yaml
vibe: "intense parkour dash with water splash"  # 旧字段，依然可用
action: dash                                     # SESSION-201 显式步态
reference_image: /home/ubuntu/refs/parkour.png   # SESSION-201 IPAdapter 参考图
vfx_overrides:                                   # SESSION-201 CRD 风格开关
  force_fluid: true
  force_physics: false
```

### 30.7 三层进化循环对接（与 P1-SESSION-202 的接口预埋）

SESSION-201 把"用户意图"升级为强类型契约后，下游 `P1-SESSION-202-POST-RENDER-ANALYSIS-AND-VIBRATION-FILTERING` 只需要在产出回流时读取 `spec.to_dict()["vfx_overrides"]` 与 `spec.to_dict()["action_name"]` 即可决定 SSIM 帧间稳定性体检的阈值与防抖滤波的强度（流体强制开 → 容忍更高的湍流方差）。当前架构已经把 `vfx_overrides` 写进 `as_admission_payload()`，所以序列化通过 PDG 工人节点后字段不会丢失，SESSION-202 直接接入即可。

### 30.8 强制守护红线（自检表）

| 红线 | 状态 |
|------|------|
| 反业务下沉：CLI 仅做意图收集 / 路径预检 / 参数组装 | ✅ |
| 反空跑宕机：`os.path.exists()` 即时校验 + 5 次重试上限 | ✅ |
| 反重度依赖：仅使用标准库 `builtins.input()` + ANSI 高亮 | ✅ |
...
| 参考 | 应用 |
|------|------|
| Vue CLI Creating a Project (cli.vuejs.org) | 渐进式问答 + 缺省值默认回车通过 |
| Vercel CLI / GitLab CLI `--yes` flag | Headless 自动化静默逃生门契约 |
